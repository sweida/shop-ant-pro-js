  ...
  if (table.columns) {
    table.columns.map((column: any, key: any) => {
      if (column.render) {
        ...
        column.render = (text: any, row: any) => <span>{eval(renderText)}</span>;
      }

      if (column.isImage) {
        column.render = (text: any, row: any) => <img src={text} width={40} height={40}></img>;
      }

      if (column.isIcon) {
        column.render = (text: any, row: any) => <Icon type={text} />;
      }
                        ...
                        callback(
                          ...
                          action.name == '启用|禁用'
                            ? row.status == '正常'
                              ? [row.id, '2']
                              : [row.id, '1']
                            : [row.id, action.onClick['value']],
                          action.onClick['url'] + '?id=' + row.id,
                        )
                      }
                      style={action.style}
                    >
                    ...
                    <Popconfirm
                      ...
                      onConfirm={() =>
                        callback(
                          ...
                          [row.id, action.onConfirm['value']],
                          action.onConfirm['url'] + '?id=' + row.id,
                        )
                      }
                    >
                      <Button size={action.size} type={action.type} style={action.style}>
                        {!!action.icon && <Icon type={action.icon} />}
                        {action.name}
                      </Button>
                    </Popconfirm>
                    ...
                    search.map((control: any) => {
                      ...
                      if (control.componentName == 'select') {
                        return (
                          ...
                          >
                            {getFieldDecorator(control.name, {
                              initialValue: control.value ? control.value.toString() : undefined,
                              rules: control.rules,
                            })(
                              <Select
                                size={control.size}
                      ...
                      if (control.componentName == 'datePicker') {
                        return (
                          ...
                          >
                            {getFieldDecorator(control.name, {
                              initialValue:
                                !!control.value && moment(control.value, control.format),
                              rules: control.rules,
                            })(
                              <DatePicker
